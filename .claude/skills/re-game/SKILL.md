...
反作弊游戏分析默认沙箱 + 隔离（[[re-analyze/platform-tips]] 最高原则；[[re-sandbox]] 强制前置）；动态调试按 OS 分支。
...
- Linux: [[re-gdb]]（Wine 下跑 Windows 游戏时读 Wine 进程内存，见 [[re-analyze/platform-tips]] Linux 分支 Wine 直读方案）
...
5. **反作弊对抗（检测规避，沙箱内）**：
   - 认知: EAC/BattleEye 检测调试器存在、内存改写痕迹、CE/外挂痕迹、多开/虚拟化特征——分析环境与真实在线环境完全隔离（专用 VM + 快照，[[re-sandbox]] + [[re-analyze/platform-tips]] 最高原则）
...
- [[re-sandbox]]：反作弊游戏分析强制隔离（[[re-analyze/platform-tips]] 最高原则）
...
- **反作弊（EAC/BE）强检测**：现象——attach 被拒、调试器被检测、账号被封；原因——反作弊驱动级监控调试器与内存痕迹；对策——分析环境完全隔离（专用 VM/离线，[[re-sandbox]] + [[re-analyze/platform-tips]] 最高原则），检测点静态分析后绕过（[[re-anti-analysis]]），绝不在在线环境挂调试器
...
- **IL2CPP 双文件结构配对（metadata 骨架 + 二进制血肉）**：现象——只有 `global-metadata.dat` 或只拿二进制，dump 结果缺方法实现/字段偏移/类型图，两份文件版本错位时解析错乱；原因——IL2CPP 产物分两半：metadata（magic 0xFAB11BAF）只存声明型骨架（类型/方法/字段定义、参数默认值、字符串字面量、泛型容器、接口偏移、vtable 方法索引），二进制（GameAssembly.dll / libil2cpp.so 等）存血肉（Il2CppType 类型图、字段偏移表、方法指针表、泛型实例表），类型引用跨两侧互链，任一缺失或版本错位都无法完整还原；且信息归属随版本漂移（v27 移除 metadata usage、新版本又把 method specs 等迁回 metadata——具体迁移版本以实测为准，别按记忆中的版本号写死）；对策——两份文件同版本配对使用；信息归属按版本分叉处理；二进制侧解析只依赖「虚拟地址→文件偏移映射 + 指针大小」两个原语，PE/ELF/Mach-O/WASM/NSO 统一抽象
（来源：Cpp2IL（SamboyCoding），MIT）
- **metadata 版本判定要靠 version 号 + Unity 版本联合裁决**：现象——dump 工具对同一 version 号在不同游戏上时好时坏，结构解析错位（字段读歪、索引越界）；原因——头部 version 字段是粗粒度信号，同一版本号对应多代结构（24 细分为 24.1~24.5、27 分 27.1/27.2 等），必须用 Unity 版本号二次裁决才能正确读结构；不同产品线还可能同号不同义（换皮陷阱），需按版本走分支；Unity 6 起（v38+）类型/方法/字段索引不再固定 4 字节，而是 8/16/32 位动态宽度，宽度从 section 的 Size/Count 反推，类型索引宽度甚至要从二进制侧 interfaceOffsets 结构推导（类型表不在 metadata 里）；对策——version 字段 + Unity 版本联合裁决，按「引入变更的 Unity 版本」建版本映射表并记录每代变更点（字段增删、位宽变化、结构移动），hotfix 版本号有时是关键分界；索引变宽后先反推元素宽度再读表，不写死 4 字节
（来源：Cpp2IL（SamboyCoding），MIT）
- **注册结构定位：计数模式搜索 + 候选仲裁**：现象——在二进制里搜 MetadataRegistration/CodeRegistration 结构，假命中一堆，按第一个解析全是垃圾；原因——这两个注册结构（MetadataRegistration：类型/字段偏移/metadata usage 的指针与计数表；CodeRegistration：方法指针表、泛型方法指针表、codegen module 指针表）是 dump 的第一步，没有名字可依，只能以 metadata 已知计数（方法数/类型定义数）为字节模式全文件搜索，假命中极多；对策——命中处按结构体解析后用验证规则集排除假命中：count 字段不超上限（方法数/类型数十万量级）、指针字段必须能映射回文件内、关键计数与 metadata 侧交叉一致（fieldOffsetsCount == 类型定义数、numTypes 不小于类型定义数）；上限阈值随样本库持续更新（真实样本把某计数推高到数十万量级）；v24.2+ 可走 xref 引用链（mscorlib 字符串 → 指向它的 codegen module 指针 → 模块列表 → 回退到 CodeRegistration 基址）；全部失败时降级人工输入地址
（来源：Cpp2IL（SamboyCoding），MIT）
- **metadata 被篡改/加密的静态对抗**：现象——metadata 解析失败（magic 不符、section 数对不上、布局乱），只想到运行时 dump 一条路；原因——反作弊/自定义保护可能加密或打乱 metadata 头部；对策——先定 Unity 版本再解析，版本来源有优先级与互验（globalgamemanagers 优先 → data.unity3d 包文件 → 主程序版本，出现可疑值自动换备用来源重试）；加密密钥用「已知明文特征」反推而非爆破——头部 offset/length 字段高字节必为 0（文件小于 32MB）、字符串区以两个 0 字节开头、头部字段是顺序字节流（XOR 后相邻字节差固定），约束收敛后逐段校验确认；布局被打乱时把头部所有候选 offset/length 当池子 DFS 拼接重建 section 链（偏移与当前位置差在合理区间、长度使位置推进且不越界、对齐约束）；解析失败可挂 fixup 回调（原始字节 + Unity 版本 → 返回修复后字节重读），这是对抗加密/混淆 metadata 的挂载点
（来源：Cpp2IL（SamboyCoding），MIT）
- **AOT 泛型实例不完整 + 共享实现**：现象——反编译里某些泛型方法没有实现或地址是占位值，不同泛型调用点还原出同一实现、类型对不上；原因——AOT 只生成游戏实际用到的泛型实例，未用实例指针为占位值；值类型参数与引用类型参数按尺寸分组共享实现（List<某引用类型> 与 List<object> 共享同一实现指针，List<int> 单独实现）；编译器去重/共享方法体使「地址→方法」多对一；对策——从调用地址反查 ConcreteGenericImplementationsByAddress 列出该地址对应的全部泛型实例，这是还原泛型调用的关键通道；共享体还原用保守仲裁（仅当恰好一个非静态候选的声明类型匹配接收者类型才定案，宁可不解不猜）；共享泛型体调用时额外隐藏传 MethodInfo* 尾参，还原时注意别把隐藏参数当普通参数
（来源：Cpp2IL（SamboyCoding），MIT）
