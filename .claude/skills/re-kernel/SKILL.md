...
- **系统设置界面操作 → ProcMon 追底层命令行（权限弹框绕过）**：现象——最新 Win11 注册表直接改（ConsentStore Value=Allow）失效，网上方法全不行；原因——设置项已由独立服务（如 camsvc，`-k osprivacy -p -s` 服务）托管，注册表改动被服务覆盖/校验；对策——ProcMon 监控 SystemSettings.exe（**注意进程名含模糊匹配**，设置操作会拉起子进程）→ 发现实际执行体与命令行（如 `SystemSettingsAdminFlows.exe SetCamSystemGlobal location 1`）→ 该命令行即"免界面调用设置流程"的官方入口，直接命令行执行即可绕过弹框；通用方法论：设置 UI 的每个操作最终都落到某个可命令行调用的系统组件，ProcMon 追进程启动（Process Start 事件）+ 命令行参数即找到自动化入口
