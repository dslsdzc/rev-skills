...
- **动态注册函数地址不在导出表**：现象——`readelf --dyn-syms` / strings 里找不到 `Java_*` 或业务函数名，IDA 里全是地址没名字；原因——动态注册时函数是静态/局部符号，运行时才由 `RegisterNatives` 把地址与 Java 方法绑定；对策——先定位 `JNI_OnLoad`（导出表一定含它），反编译看 `RegisterNatives` 第三参数数组逐项还原（name/signature/fnPtr 各 8 字节）；或 frida spawn 后 hook RegisterNatives（步骤 3 脚本）直接拿运行时注册表
