...
4. 仿真：[[re-fw-emulate]]（能力：`emulation`） —— 需要运行时用 QEMU 用户态优先（最轻方案，见 [[re-analyze/platform-tips]]），全系统按需
...
- 仿真动态分析默认沙箱 + 网络隔离（[[re-analyze/platform-tips]] 最高原则）
...
- 仿真不隔离网络就跑固件 → 固件真实回连外网 —— 默认沙箱 + 网络隔离（[[re-analyze/platform-tips]] 最高原则），通信分析转 [[re-protocol]]
