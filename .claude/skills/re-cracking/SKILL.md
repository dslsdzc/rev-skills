...
3. **分派**：根据校验算法性质与目标选择路径——
   ...
   - 算法不可逆（哈希 / 非对称验签）→ **[[re-patching]]**（跳过验签 / 改判定分支）；需要从比较链 / 数学关系硬推序列号 → z3-solver 建模可选（re-z3 技能后续建立，先用 `pip install z3-solver`，见坑 4）
...
- **校验算法不可逆（哈希 / 非对称签名）** → [[re-patching]]（跳过验签）；想从比较链硬推 → z3-solver 建模（re-z3 后续建立）
...
- **盲目追求注册机**：现象——校验算法含 SHA / MD5 或 RSA 验签，注册机写不出来，方案卡死；原因——不可逆部分是单向的，正推生成序列号不可能；对策——按第 3 步分派：不可逆改 [[re-patching]] 跳过验签；必须约束求解时用 z3-solver 只对可逆部分建模
