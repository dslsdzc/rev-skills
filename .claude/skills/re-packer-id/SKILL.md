...
- **壳套壳**：现象——识别为 UPX 但 `upx -d` 解包后 `file` 仍报壳特征；原因——多层加壳（UPX 里包 Themida 等）；对策——脱一层重识别一层，**逐层循环复查四要素：PE header（新 EP 位置 / 节表结构 / 头字段）、节名（是否出现新壳节特征）、熵分布（是否有新高熵节）、导入表（是否仍极小 / 只剩 GetProcAddress 动态调用）**——四要素全部正常才算脱净，任何一项异常就回到本技能重跑
- **识别阶段不记 OEP 线索**：现象——脱壳到一半需要"壳入口指令 / EP 地址"时手忙脚乱，断点位置全靠猜；原因——识别时没记录；对策——步骤 7 在识别阶段就记录入口指令（pushad 等）与地址，供 ESP 定律直接使用
- **识别出壳名 ≠ 壳配置全开**：现象——DIE 报 VMProtect/Themida 就默认走强壳全套流程，实际样本只是简单压缩或虚拟化未启用；原因——壳名只表示"加了什么壳"，是否启用虚拟化/反调试是构建时的配置选项；对策——识别报告带上版本与配置线索（`.vmp0`/`.vmp1`/`.themida` 节是否存在、入口是 VM 分发型还是普通 stub、位数），交由 [[re-anti-analysis]] 决定走 [[re-unpack-simple]] 还是 [[re-unpack-advanced]] 流程
- **自动化脱壳工具对版本/位数敏感**：现象——x64Unpack/Magicmida 等自动化工具对目标无效、dump 损坏；原因——脱壳器按壳的具体版本与位数实现（Magicmida 仅支持 32 位且不修 VM 反转储；x64Unpack 只验证过 Themida 2.4.6/VMProtect 3.4）；对策——识别阶段记录壳名 + 版本 + 位数（DIE 版本字段/深度扫描），选工具前核对支持矩阵，不支持就走手动流程
