...
1. **模型提取攻击**：
   ...
   - 步骤：
     1. 确认授权与用途（评估/取证声明记录）
     2. 接口探测：读 API 文档/试查询 → 确认限额与输出字段（logits/概率或仅标签？置信度可得性决定策略）
     3. 查询采样（输入分布覆盖：代表性输入 + 边界扰动 + 标签分布均衡）
     4. 蒸馏训练（以 API 输出为教师标签，soft label 优于硬标签）
     5. 评估近似度（同输入集输出对比：一致率 + 标签分布对比，不是只看精度）
   - 边界扰动：输入空间小幅扰动（噪声/旋转/亮度）——覆盖决策边界附近区域
   - 评估一致性：同一输入多次查询（logits 方差）——方差大说明输出随机化/防提取机制
   - 蒸馏训练要点：学生模型容量与教师相近；训练轮次/学习率按小验证集调
   - 蒸馏评估基线：随机初始化学生模型与目标模型同分布对比（排除「模型本身输出相似」假象）
   - 查询日志：每次查询记录（输入指纹/输出/时间/配额消耗），供审计与复现
   - 仅标签时：降级为决策边界采样（输出分布重建受限——标注此局限，见 [[gotchas]] 接口坑）
   - 查询预算内先小批估计（限流/计费中断处理见坑 1）
...
2. **模型指纹/水印检测**：
   ...
   - 抹除对抗：剪枝/微调/蒸馏会削弱指纹——用鲁棒指纹（多触发器）并标注「可能被抹除」（见坑 3）
...
3. **成员推断**：
   - 目标：判定某样本是否在训练集中
   - 输入形态：目标模型输出的概率向量 + 样本特征
   - 方法：过拟合特征（样本损失/置信度分布对比基线）、影子模型（同分布代理模型校准）
   - 攻击类型选择：白盒（有 loss）用阈值法；黑盒（只有输出）用影子模型
...
4. **对抗样本基础**：
   - 白盒：梯度扰动（FGSM/PGD 类方法）——前提是模型权重可加载且 loss 可微（拿到文件但无法加载时退回黑盒路径）
   - 黑盒：查询/迁移扰动
