...
3. **简单壳脱壳：[[re-unpack-simple]]（能力：`unpack`）** —— 优先官方/自动解包（`upx -d`）→ ESP 定律 / 内存断点找 OEP → **OEP 解密完成后转储**（时机见 [[re-analyze/platform-tips]] 关键经验，默认转储优先）→ IAT 修复（Scylla/ImpREC，Windows）
...
## 反虚拟化方法论（全库通用，见 [[re-analyze/analysis-contract]] 索引）
...
- 脱壳产物验证必须沙箱：[[re-sandbox]]（[[re-analyze/platform-tips]] 最高原则）
...
- **转储时机过早**：现象——dump 出来还是壳的初始状态（压缩/加密数据），脱了个寂寞；原因——壳未运行到 OEP 就转储；对策——等解密完成后 dump（[[re-analyze/platform-tips]] 关键经验，默认转储优先）
...
- **高熵≠加密**：现象——`.of2` 这类节 4KB 熵 7.9 判"整体加密"，静态放弃；原因——可能是压缩（deflate 无标准头）、VMProtect 变异代码、或纯数据；对策——**分区采样**（入口区 64B 熵可能只有 5.6 是真代码），先排除压缩与变异再谈加密；变异代码特征：`ror/xor 链 + 跨区 jmp`、顺序解码出现 `fcom/int 88h/ret far` 类垃圾但入口 24 字节是 call/syscall 结构
- **调试体系全链路（反调试点位）**：现象——不理解"附加为什么让程序暂停"、不知道反调试该改哪里；原因——缺 Windows 调试体系全链路认知；对策——链路：`DebugActiveProcess`→`DbgUiConnectToDbg`（创建 DEBUG_OBJECT 先挂调试器，句柄存 `TEB->DbgSsReserved[1]`（偏移 0x16A8））→`NtDebugActiveProcess`→`DbgkpSetProcessDebugObject`（被调试进程挂上）+`DbgkpPostFake*Messages`（附加时刷屏的"创建线程/加载 dll"假消息）；调试事件流：被调试进程检测被调试→创建 DEBUG_EVENT 挂入 EventList 双向链表→置位 EventsPresent 信号量唤醒调试器→死等 ContinueEvent；**反调试点**：`DbgUiRemoteBreakIn` 创建远程线程执行 int3 让程序断下（去掉该 int3 则附加断不下来）；TEB+0x16A8 是调试对象句柄（检测/篡改干扰调试器）；异常先发给调试器（`DbgkSendApiMessage`→`DbgkpQueueMessage`，flag=0 需等待 vs NOWAIT 假消息）
- **so 函数级加密（dlsym 运行时解密）**：现象——so 里个别函数（如 `mymyadd`）反编译是乱码/未定义指令，其余正常；原因——so 加固把目标函数按符号名加密：函数代码段静态是密文，运行时 `dlopen`+`dlsym` 前先解密覆盖、调用完再加密回去；对策——还原思路：dlsym 符号解析链（`__loader_dlsym`→`dlsym_impl`→`do_dlsym`→`dlsym_handle_lookup`→`find_symbol_by_name`，GNU hash / ELF hash 二选一：GNU hash 需算 `gnu_hash` + 布隆过滤器 + 从 PT_DYNAMIC 找符号表），按 ELF 头→程序头表→PT_DYNAMIC 定位符号表与哈希表计算函数位置与大小；动态侧：断 `dlopen`/`dlsym` 返回后、解密后的函数内存处 dump（转储时机关键经验）；变体：段头 type 逐段 XOR（MODE_DUPLEX）需要自定义 linker 加载才能跑；还有对 `.text` 整体加密后自解密——先确认是函数级还是段级再选方案
