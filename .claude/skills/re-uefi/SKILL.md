...
### UEFIFind（按 GUID/类型定位文件，随 uefitool-cli）
...
- .efi 模块是 PE/COFF（subsystem EFI_BOOT_SERVICE_DRIVER / EFI_APPLICATION），两家都能直接导入；Ghidra 11.1+ 另有 UEFI 分析扩展
...
- Fedora: `dnf install edk2-ovmf`（同路径）
...
- 验证: `ls /usr/share/OVMF/OVMF_CODE.fd`（macOS 用 brew 前缀路径）
...
2. **DXE 驱动提取**：
   ```sh
   ueifind -t DXE fw.bin             # 按文件类型定位 DXE 驱动
   ueifind -g <GUID> fw.bin          # 已知 GUID 直接定位
   # 提取指定文件: uefiextract fw.bin <GUID> -o driver.efi -m extract -t PE32
...
4. **bootkit 定位（SMM / 定时器回调 / 启动路径挂钩）**：
   ...
   - **SMM**: `SmiHandlerRegister` / SMST 服务 → SMM driver 注册 SMI handler（SMM 内执行的代码，ring0 之前的持久层）
...
- **PE 头在 FFS 内偏移**：现象——从固件里抠出的"文件"直接 file/IDA 打开失败或反编译全是乱码；原因——FFS File Header（24/32 字节）+ Section Header + 对齐填充后才是 PE32 主体，PE 头不在文件偏移 0；对策——用 UEFITool 右键 Extract body / `uefiextract ... -m extract` 导出裸 PE，导出后 `file` 确认输出含 "PE32+ executable (EFI boot service driver)" 再进反编译器；手工提取要按 Section 布局算偏移
