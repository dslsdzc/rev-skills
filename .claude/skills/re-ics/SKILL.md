...
1. **协议识别（端口 + 特征）**：
   ...
   ```
   - 端口速查：Modbus/TCP 502（Modbus over TLS 802）、DNP3 20000/TCP 20001/UDP 20002/TLS、OPC UA 二进制 4840/TCP、S7comm 102、EtherNet/IP 44818、BACnet/IP 47808/UDP
...
4. **scapy 解析器（modbus）+ 自定义 DNP3 解析**：
   ...
   ```
   - DNP3（scapy 无层）：链路层头结构固定（`05 64 | len | ctrl | dst(2) | src(2)`），用 struct 解析即可；传输层/应用层按 IEEE 1815 的组-变体格式递进
...
- [[re-crypto-id]] / [[re-crypto-keys]] / [[re-crypto-decrypt]]：加密变体前置（Modbus over TLS、DNP3 Secure Authentication、OPC UA 安全通道）
...
- **协议变体混用**：现象——dissector 未命中或解析错位（Modbus RTU 帧被当 TCP 解、DNP3 走 TLS 端口）；原因——同一协议多载体：Modbus RTU/ASCII（串口）vs TCP（502）vs TLS（802）；DNP3 串口/20000/20002；OPC UA 二进制（4840）vs HTTP 端点；对策——步骤 1 端口+特征双确认，Wireshark Decode As / `-d` 强制指定 dissector
- **加密变体当明文解**：现象——流量熵高、dissector 只出乱码或标记加密；原因——Modbus over TLS / DNP3 Secure Authentication / OPC UA 安全通道默认加密；对策——先 [[re-crypto-id]] 判定，密钥从 HMI/客户端进程内存与证书提取（[[re-crypto-keys]]），解密链路走 [[re-crypto-decrypt]]
