...
- **入口调度**：本网关被 [[re-analyze]] 的 triage.md「网络协议逆向 / 分析通信加密」路径调用（re-protocol → netcap → crypto-* → proto-rev）
...
- **抓包不过滤 → 文件巨大没法分析**：现象——pcap 几十 GB，tshark 统计卡死；原因——抓包点没过滤（全接口全协议）；对策——[[re-netcap]] 步骤 3 先写过滤表达式（host/port/协议），先过滤再存盘
