...
- **声明式定义优先，抓包兜底**：现象——协议还原从抓包/行为观察开始，绕大圈；原因——客户端常内嵌声明式定义（proto/schema/接口描述文件），明文或半明文可提取；对策——解包先搜声明式定义（`syntax = "proto3"`、`.fbs`、接口描述等），**还原优先级：声明式定义 > 反编译打包/解包实现 > 抓包**
- **运营中的官方接口直接探测**：现象——静态猜响应格式反复试错；对策——官方服务还在运营时，按客户端请求参数直接打真实接口（只读类、低频率），信封（`{code,msg,data}`）与错误码语义（401 参数错/404 id 错/406 op 错）一次拿全，再对照客户端解析代码确认字段；比纯静态还原快一个数量级
- **加密"未实现"要全量搜索确认**：现象——协议 flag 位存在 `0x40000000=Encrypt` 就假定有加密层；原因——flag 位是基础设施，功能可能从未启用；对策——全量搜索算法实现（AES/RC4/XXTEA 等）与调用点，C#/原生层都找不到才下"未加密"结论（常见情形：加密位是预留，压缩位才实际使用）
