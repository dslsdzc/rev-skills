...
1. **分组统计与聚类（长度/方向/时序）**：
   ...
   ```
   - 聚类目标：按方向（A→B / B→A）分组看长度分布；长度呈少量固定值 → 固定长度消息（可能无长度字段）；长度连续变化 → 变长消息（通常需要某种 framing，候选：内嵌长度字段、分隔符、固定语法、chunk 分块、连接关闭）
...
4. **Scapy 写解析器（自定义协议类）**：
   ...
   pkt = C2(bytes.fromhex("aa5501000a" + "54657374696e672e2e2e" + "00" * 54))   # aa55 | 01 | 000a | "Testing..."（10B）+ 54B 零填充，凑满 64B payload
   pkt.show()
   ```
...
- **重注入后对端报 FCS 错 / 帧长不对**：现象——把抓到的帧重注入后，对端 Wireshark 报 "ETHERNET FRAME CHECK SEQUENCE INCORRECT" 或帧长与预期不符；原因——**要分开看两个长度**：以太网 **FCS 是 4 字节**，而**典型以太网头部是 14 字节**（6 目的 MAC + 6 源 MAC + 2 类型）——**"短 14 字节"指向的是头部/链路层封装问题，不是 FCS**；抓包库的帧抽象（Padding 处理、FCS 字段是否解析、是否带 trailer）会因版本与配置而异，**不能笼统归结为"不保留尾部"**；对策——先量出**实际少了多少字节**再定位：缺 4 字节查 FCS 是否被剥离，缺 14 字节查是不是丢/重复了以太网头或注入到了错误的层；重注入优先用**原始字节**而非 round-trip 后的包，或在本机环回（lo）用 L3 注入避开以太网层
...
- **Java 客户端忽略 JNI 契约**：现象——原生库逆向找不到入口；原因——Java 层通过 JNI 调原生，静态注册的方法会以 `Java_包名_类名_方法名` 留在动态导出表（strip 不删 .dynsym）可作接口文档；但 `RegisterNatives` 动态注册的库**没有** `Java_*` 导出；对策——`nm -D` 列 JNI 导出，有则函数名直接给契约（encode/parse/setKey 分工）从导出函数开始逆向；无则从 `JNI_OnLoad`/`RegisterNatives` 调用点、JNI 函数表 xref、字符串与运行期 hook（[[re-frida]]）定位入口
...
- **数据无规律 → 先验加密**：现象——每帧都不同；原因——压缩/加密层；对策——熵分析（>7.5 → 加密/压缩/高随机性候选，结合帧长度规律与熵分布佐证，高熵不是判定），加密层找 nonce/IV 字段；长度字段解不出时按「长度可能 little/big-endian、含/不含自身」列方程组解语义
- **重放被拒**：现象——数据正确但服务端不响应；原因——协议带递增 seq/nonce；对策——搞清楚 seq 计算方式（前一帧 hash 或递增计数器），复现时正确推进
- **闭环验证**：现象——协议还原是否成立无判据；原因——静态分析无法自证还原正确；对策——用还原的帧结构写 client 发一帧，服务端响应一致才算成立
