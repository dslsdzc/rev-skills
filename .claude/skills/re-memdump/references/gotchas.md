...
- **转储过早 = 壳的初始状态**：壳解密前 dump 拿到的是压缩/加密数据——脱壳样本必须等 OEP 后（SKILL.md 坑 2；[[re-analyze/platform-tips]] 关键经验）
...
- 动态运行样本在沙箱内执行（[[re-analyze/platform-tips]] 最高原则）
- 转储产物与时间戳 sha256 存档（[[re-triage]]）；结论写 [[re-analyze/analysis-contract]]
