...
1. **DRM 组件识别（CDM / 解密器）**：
   ...
   - CDM 组件：`libwidevinecdm.so`（Chrome 内置，`-arm` 为 L1 变体、`-x86`/`-x64` 为 L3 软件实现——**架构与等级相关，见坑 1**）；PlayReady 组件在 Edge/Windows 系统组件；FairPlay 在 Apple 侧（[[re-ios]] 域）
...
- **L1/L3 等级差异（硬件 vs 软件）**：现象——同一个 Widevine CDM 相关样本，在 A 设备能分析的密钥结构/流程在 B 设备完全对不上，或按 L3 思路分析 L1 目标处处碰壁；原因——安全级别不同：L1 密钥在 TEE（可信执行环境）硬件中保护，L3 是纯软件 CDM 可提取可分析；架构标识：`libwidevinecdm.so` 的 `-arm` 变体对应 L1、`-x86`/`-x64` 变体是 L3；PlayReady 同样分硬件信任根与软件实现；对策——先确认目标安全级别（CDM 变体/设备能力报告/厂商文档），L3 才能做软件层密钥提取分析，L1 分析走 TEE/信任根方向（硬件安全）；结论严格标注级别，不跨级别套用
