...
- **字符串可见 ≠ 答案（诱饵 flag）**：现象——`strings` 直接看到 flag/密钥/关键串，提交却不对；原因——高难度样本故意把诱饵放"分析师最容易看到且不会怀疑"的位置（DOS stub、资源段、明显字符串），真实答案藏在协议逻辑/加密数据里；对策——字符串只是线索不是结论，必须交叉验证（代码引用、协议逻辑、多份副本对比）；**越容易看到的位置越可疑**，尤其 DOS stub（PE 头里）出现"flag 格式"字符串基本是陷阱
