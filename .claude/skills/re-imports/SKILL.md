...
- **运行时的 GOT/IAT 可能已被构造器改写**：现象——静态导入表干净，运行后却调用异常函数；原因——恶意构造器（`.init_array`/TLS 回调）在入口之前覆写 GOT/IAT 条目（如 `strcmp@GOT`）装 hook，静态视图看不到；对策——导入结论与运行内存视图对照（[[re-memdump]]/[[re-tracing]]），样本带初始化回调时先把回调列出来再下结论
- **家族聚类用 imphash 而非导入名列表**：现象——同家族变种导入名差异大，按名字聚类失败；原因——导入名列表对链接器版本/加壳敏感；对策——用 pefile 的 `imphash`（导入函数归一化后哈希）做指纹；注意 IAT 被壳接管的样本 imphash 失真，先脱壳再算
