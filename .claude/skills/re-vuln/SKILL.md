...
按顺序执行；每步产物（目标/输入面结论、fuzz 运行目录、崩溃样本、定位证据）记录证据路径 + sha256（存证方法见 [[re-triage]]（能力：`triage`）），供报告引用。
...
1. **目标与输入面识别** —— 先弄清"fuzz 什么、输入从哪进"（见坑 1）：
   ...
   - 初勘：`file` / `strings` / 熵 / checksec（[[re-triage]]（能力：`triage`））；静态定位解析入口（[[re-binary-core]]（能力：`decompilation`、`debugging`、`memory-dump`、`elf-parser`、`pe-parser`、`macho-parser`） 底座，[[re-ghidra]]（能力：`decompilation`、`debugging`） 反编译找主处理函数）
   - 决策：fuzz 引擎与插桩方式（源码可插桩 / 无源码 QEMU 模式），见「选择树」
2. **覆盖率引导 fuzzing → [[re-fuzzing]]（能力：`fuzzing`）** —— 插桩 + 语料 + 跑起来：
...
3. **崩溃分析 → [[re-crash-triage]]（能力：`debugging`）** —— 出现 crash 立即处理：
   - 确定性复现（同一输入重跑必崩）；ASAN 报告解读（堆溢出 / 越界 / UAF / double-free）
   - 输入最小化（afl-tmin / afl-cmin）→ 最小 PoC
   - 非确定性崩溃用 rr 录制重放
4. **逆向定位（[[re-binary-core]]（能力：`decompilation`、`debugging`、`memory-dump`、`elf-parser`、`pe-parser`、`macho-parser`） 底座）** —— 把崩溃翻译成漏洞根因：
   - [[re-triage]]（能力：`triage`）：崩溃输入 sha256 存证 + 目标基线记录
   - [[re-ghidra]]（能力：`decompilation`、`debugging`；或 [[re-ida]]（能力：`decompilation`、`debugging`） / [[re-radare2]]（能力：`decompilation`））：gdb 回溯栈中无符号帧用反编译补（见坑 5）；定位崩溃函数与输入数据流向
   - 确认：越界/悬垂发生在哪段逻辑、输入是否可控、路径是否真实可达（见坑 3）
   - 崩溃 core（进程已死 / 已有 core dump）→ [[re-memdump]]（能力：`memory-dump`） 分析
5. **报告**：
   - 记录：崩溃类型、复现命令、最小 PoC（文件 + sha256）、根因（函数/行/偏移）、可达性分析、影响（越界读/写、RCE/DoS）、修复建议
   - 产出：PoC / 崩溃特征可进 [[re-ioc]]（能力：`threat-intel`）

## 何时用哪个原子技能（选择树）

按目标输入形态 / 当前状态分支：

- **本地文件解析**（输入是一个文件：图像/压缩/文档/自定义格式解析器）→ [[re-fuzzing]]（能力：`fuzzing`） 用 **AFL++**（源码插桩 + ASAN；无源码 `-Q` QEMU 模式）
- **库函数**（目标是库的 API，如 libpng 的 `png_read_*`、自定义 parser 函数）→ [[re-fuzzing]]（能力：`fuzzing`） 用 **libFuzzer**（`-fsanitize=fuzzer,address` + fuzz target harness），或 AFL++ 持久模式 harness
- **网络服务**（监听端口、协议解析）→ 网络 fuzz：[[re-netcap]]（能力：`network-capture`） 抓真实流量做种子，写 harness 重放喂解析函数（见 [[re-fuzzing]]（能力：`fuzzing`） 坑 5）；fuzz 的是解析器不是网络栈
- **已有崩溃 / 崩溃输入**（无需重跑 fuzz）→ 直接 [[re-crash-triage]]（能力：`debugging`）：确定性复现 → 最小化 → 定位
- **崩溃已定位、要读函数逻辑** → [[re-binary-core]]（能力：`decompilation`、`debugging`、`memory-dump`、`elf-parser`、`pe-parser`、`macho-parser`） 底座（[[re-ghidra]]（能力：`decompilation`、`debugging`） / [[re-ida]]（能力：`decompilation`、`debugging`） / [[re-radare2]]（能力：`decompilation`））
- **崩溃是 core 文件**（进程已死）→ [[re-memdump]]（能力：`memory-dump`） 分析 core，再按需逆向
- **目标无输入解析（纯算法/纯逻辑）** → fuzz 不适合，符号执行更优（[[re-angr]]（能力：`symbolic-execution`） / [[re-z3]]（能力：`constraint-solving`））
...
- **动态执行沙箱**：fuzz / 复现崩溃都是动态执行，默认在 [[re-sandbox]] 内跑（[[re-analyze/platform-tips]] 最高原则）
