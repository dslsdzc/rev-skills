...
- **架构分支**：arm64 SYS_OPENAT=56（无 open 系统调用）、svc 机器码 `01 00 00 D4`，路径参数在 x1（x0 是 dirfd）；arm SYS_OPEN=5、`00 00 00 EF`，路径在 r0
- **手法**：遍历可执行段（r-x）过滤 .so → 特征码扫描命中 → 回读 svc 前一条指令取系统调用号（arm64 读 `mov x8` 立即数位、arm 读 r7）→ 系统调用号等于目标（如 SYS_OPEN）才 attach → 打印文件名与返回值
- **注意**：arm64 参数索引按系统调用约定核对（openat 路径参数在 x1，x0 是 dirfd，不照搬经验值）；系统调用号表按目标内核/Android 版本核对
