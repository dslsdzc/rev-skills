...
| 方法论 | 位置 | 适用场景 |
|---|---|---|
| 分析方法论（入口/调用/内存/API/混淆/动态调试/深层/验证/知识获取，9 组） | [[re-binary-core]]「分析方法论」 | 一切二进制分析的通用手法（静态为主） |
| 反调试方法论（基础检测/异常机制/时间检测/环境特征/硬件痕迹/线程与调试对象/自修改断点/深层检测，9 组） | [[re-anti-analysis]]「反调试方法论」 | 任何动态分析遭遇反调试时（gdb/x64dbg/mobile/沙箱） |
