...
2. **constants——常量定义**：识别魔数与加密常量。找什么：疑似 #define 的数字字面量、位掩码及其构成的 flag、有语义的 magic number（如 ELF 魔数——**字节序列是 `7f 45 4c 46`**，写成 C 字面量即 `"\x7fELF"`；按 u32 读时注意字节序：小端得 `0x464c457f`、大端才是 `0x7f454c46`）、协议专用常量、用作 key/标识符的字符串常量
